...
struct TestHandler : public IHandler
{
    ...
    std::shared_ptr<ICommand> getCommand([[maybe_unused]]std::shared_ptr<ICommand> cmd, [[maybe_unused]]std::exception_ptr ex) override
    {
...
TEST(GameServer, ExceptionHandler)
{
    ...
    std::exception_ptr ex_ptr = std::make_exception_ptr(TestException());
    auto ww = ExceptionHandler::Instance().getCommand(std::make_shared<TestCommand>(), ex_ptr);
    ww->Execute();
    ASSERT_TRUE(flag);
}
...
TEST(GameServer, LogCommand)
{
    ...
    auto ex = std::make_exception_ptr(TestException());

    auto lc = LogCommand(tc, ex, log);
    lc.Execute();
    auto qq = log->GetLogMessage();

    ASSERT_TRUE(
            log->GetLogMessage().find("Logging exception: Test exception  from command type: ") != std::string::npos);
...
TEST(GameServer, RetryCommand)
{
    ...
    {
        ASSERT_THROW(RetryCommand(tc).Execute(), TestException);
    }
}

TEST(GameServer, RetryCommandHandler)
{
    ExceptionHandler::Instance().RegisterHandler<TestCommand, TestException>(std::make_unique<RetryHandler>());
    CommandQueue::Instance().RegisterCommand(std::make_unique<TestCommand>());

    ASSERT_THROW(CommandQueue::Instance().Execute(), TestException);
}

TEST(GameServer, DoubleRetryCommandHandler)
{
    const auto log = std::make_shared<Log>();
    ExceptionHandler::Instance().RegisterHandler<TestCommand, TestException>(std::make_unique<RetryHandler>());
    ExceptionHandler::Instance().RegisterHandler<RetryCommand, TestException>(std::make_unique<LogHandler>(log));
    CommandQueue::Instance().RegisterCommand(std::make_unique<TestCommand>());
    CommandQueue::Instance().Execute();
    auto rr = log->GetLogMessage();
